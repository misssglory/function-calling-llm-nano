...
def build_file_tree(root_path, exclude_folders):
    ...
    for rel_path, lines in temp_tree.items():
        file_tree[rel_path] = {
            "lines": lines,
            "bar_length": calculate_bar_length(
                lines, max_lines_in_tree, args.max_bar_length
            ),
...
def print_tree(file_tree, total_lines, max_lines, max_bar_length):
    ...
    print("=" * 70)
    print(
        f"Max bar length: {max_bar_length} chars | Each '█' ≈ {max_lines/max_bar_length:.1f} lines"
    )
    print("=" * 70)
...
def main():
    ...
    logger.info(
        f"Excluding folders: {args.exclude_folders if args.exclude_folders else 'None'}"
    )
    logger.info(f"Excluding files: .json")
    logger.info(f"Max bar length: {args.max_bar_length} characters")

    try:
        file_tree, total_lines, max_lines = build_file_tree(
            args.path, args.exclude_folders
        )
